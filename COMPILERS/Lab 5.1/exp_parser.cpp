...
Exp* Parser::parse() {
  ...
  // Exp* e = new NumberExp(0);  // default - remover!!
  
  Exp* e = parseExpression();  // deberia ser Exp* e = parseExpression()
...
Exp* Parser::parseExpression() {
    ...
    while(match(Token::PLUS) || match(Token::MINUS)) {
        ...
        BinaryOp binop = (op==Token::PLUS)? PLUS : MINUS;
        rhs = parseTerm();
...
Exp* Parser::parseFExp() {
    ...
    if (match(Token::EXP)) {
        Exp* rhs = parseFExp();
        return new BinaryExp(e, rhs, EXP);
    }
    return e;
}
