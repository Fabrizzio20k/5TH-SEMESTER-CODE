...
void ImpInterpreter::visit(Program *p)
{
  ...
  if (!retcall)
  {
    cout << "Error: no se encontro return en main" << endl;
    exit(0);
  }
...
{
  FunDec *fd = fdecs.lookup(e->fname);
  env.add_level();
  list<Exp *>::iterator it;
  list<string>::iterator varit;
  for (it = e->args.begin(), varit = fd->vars.begin(); it != e->args.end(); ++it, ++varit)
  {
  ...
  if (!retcall)
  {
    cout << "Error: no se encontro return en funcion" << e->fname << endl;
    exit(0);
  }
